// RUN: circt-opt %s \
// RUN:   -pass-pipeline='builtin.module(firrtl.circuit(perf-insert-counter{targets=~Top|Top>x,~Top|Top>y}))' \
// RUN:   | FileCheck %s --check-prefix=INSERT

// RUN: circt-opt %s \
// RUN:   -pass-pipeline='builtin.module(firrtl.circuit(perf-insert-counter{targets=~Top|Top>x,~Top|Top>y}, firrtl.module(perf-emit-firesim-annotations)))' \
// RUN:   | FileCheck %s --check-prefix=EMIT

// RUN: circt-opt %s \
// RUN:   -pass-pipeline='builtin.module(firrtl.circuit(perf-insert-counter{targets=~Top|Top/u_mid:Mid/u_leaf:Leaf>z}))' \
// RUN:   | FileCheck %s --check-prefix=CTX-INSERT

// RUN: circt-opt %s \
// RUN:   -pass-pipeline='builtin.module(firrtl.circuit(perf-insert-counter{targets=~Top|Top/u_mid:Mid/u_leaf:Leaf>z}, firrtl.module(perf-emit-firesim-annotations)))' \
...
// INSERT:       perf.counter %x_perf : !firrtl.uint<1>, "x", %clock : !firrtl.clock, %reset : !firrtl.reset
// INSERT:       firrtl.connect %x_perf, %x : !firrtl.uint<1>, !firrtl.uint<1>
// INSERT:       %y = firrtl.wire interesting_name : !firrtl.uint<1>
// INSERT:       perf.counter %y : !firrtl.uint<1>, "y", %clock : !firrtl.clock, %reset : !firrtl.reset
...
// EMIT-SAME:  class = "midas.targetutils.AutoCounterFirrtlAnnotation"
// EMIT-SAME:  clock = "~Top|Top>clock"
// EMIT-SAME:  coverGenerated = false
// EMIT-SAME:  description = ""
// EMIT-SAME:  label = "x"
// EMIT-SAME:  opType = {class = "midas.targetutils.PerfCounterOps$Accumulate$"}
...
// CTX-INSERT:       perf.counter %z_perf : !firrtl.uint<1>, "z", %clock : !firrtl.clock, %reset : !firrtl.reset context @[[HPATH]]
...
// CTX-EMIT-SAME:  class = "midas.targetutils.AutoCounterFirrtlAnnotation"
...
// CTX-EMIT-SAME:  opType = {class = "midas.targetutils.PerfCounterOps$Accumulate$"}
